...
class Matrix:
    ...
    def multiply(self, name: str, other: Union[int, Matrix]) -> Matrix:
        if isinstance(other, int):
            rows = [[self.rows[r][c] * other for c in range(self.order[1])] for r in range(self.order[0])]
            return Matrix(name, self.order, rows)
...
def info():
    ...
    Scalar multiplication: C=kXB where k is a scalar value(eg: integer)
